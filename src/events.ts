...
export interface LyncEventBody {
  v: number;
  id: string;
  kind: string;
  at: string;
...
export interface LyncLineDiagnostic {
  file: string;
  line: number;
  class: LyncLineClass;
  reason: string;
...
export interface LyncConflictVariant {
  id: string;
  digest: string;
  file: string;
  line: number;
...
export interface LyncPendingDiagnostic {
...
export interface LyncParseResult {
  lines: LyncLineDiagnostic[];
  unionEventIds: string[];
  viewEligibleIds: string[];
  conflictIds: string[];
  conflictVariants: LyncConflictVariant[];
  pending: LyncPendingDiagnostic[];
...
export interface LyncUnionIngestResult {
  status: LyncUnionStatus;
  line: LyncLineDiagnostic;
  conflictVariants?: LyncConflictVariant[];
  missingParent?: string;
  drained?: LyncUnionIngestResult[];
}

export interface LyncUnionOptions {
  pendingLimit?: number;
}

export class LyncUnion {
  private readonly pendingLimit: number;
  private readonly lines: LyncLineDiagnostic[] = [];
  private readonly acceptedById = new Map<string, LyncLineDiagnostic>();
  private readonly conflictIds = new Set<string>();
  private readonly conflictVariants = new Map<string, LyncConflictVariant>();
  private readonly pendingByParent = new Map<string, LyncLineDiagnostic[]>();
  private pendingCount = 0;
  private pendingOverflowCount = 0;

  constructor(options: LyncUnionOptions = {}) {
  ...
  private ingestParsedLine(line: LyncLineDiagnostic): LyncUnionIngestResult {
  ...
  private acceptLine(line: LyncLineDiagnostic): LyncUnionIngestResult {
    ...
    const variants = new Map<string, LyncConflictVariant>();
  ...
  private markConflictVariant(line: LyncLineDiagnostic): LyncConflictVariant {
  ...
  private bufferPending(missingParent: string, line: LyncLineDiagnostic): void {
  ...
  private drain(parent: string): LyncUnionIngestResult[] {
  ...
  private pending(): LyncPendingDiagnostic[] {
...
export function exportCarriedLyncBytes(result: LyncParseResult): Uint8Array {
...
export function lyncDownset(result: LyncParseResult, id: string): {
  ids: string[];
  partial: boolean;
  obstacles: LyncObstacle[];
} {
  const events = new Map<string, LyncLineDiagnostic>();
...
function markConflicts(lines: LyncLineDiagnostic[]): {
  acceptedById: Map<string, LyncLineDiagnostic>;
  conflictIds: Set<string>;
  conflictVariants: LyncConflictVariant[];
} {
  const byId = new Map<string, LyncLineDiagnostic[]>();
  ...
  const acceptedById = new Map<string, LyncLineDiagnostic>();
  const conflictIds = new Set<string>();
  const conflictVariants = new Map<string, LyncConflictVariant>();
...
function validateEnvelope(value: unknown):
  ...
  return { ok: true, event: value as LyncEventBody, nonconforming };
}
...
function isUnionCandidate(line: LyncLineDiagnostic): line is LyncLineDiagnostic & {
  id: string;
  event: LyncEventBody;
  bodyBytes: Uint8Array;
  bodyDigest: string;
...
function sameBody(a: LyncLineDiagnostic, b: LyncLineDiagnostic): boolean {
  return a.bodyDigest === b.bodyDigest && bytesEqual(a.bodyBytes, b.bodyBytes);
}

function isRicherLine(candidate: LyncLineDiagnostic, current: LyncLineDiagnostic): boolean {
...
function conflictVariantFor(line: LyncLineDiagnostic): LyncConflictVariant {
...
function firstMissingParent(
  line: LyncLineDiagnostic & { event: LyncEventBody },
  acceptedById: Map<string, LyncLineDiagnostic>,
  conflictIds: Set<string>,
): string | undefined {
...
function computeSuppression(acceptedById: Map<string, LyncLineDiagnostic>, conflictIds: Set<string>) {
...
function graphObstacles(acceptedById: Map<string, LyncLineDiagnostic>, conflictIds: Set<string>): LyncObstacle[] {
  const obstacles: LyncObstacle[] = [];
  for (const line of acceptedById.values()) {
    const event = line.event;
...
function names(event: LyncEventBody): Set<string> {
