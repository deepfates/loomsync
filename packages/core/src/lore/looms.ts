...
export interface LoreLoomsOptions {
  store: EventStore;
  author: LoreAuthor;
  now?: () => number;
  createId?: () => string;
...
export function createLoreLooms<
  TPayload = unknown,
  TLoomMeta = unknown,
  TTurnMeta = unknown,
>(options: LoreLoomsOptions): Looms<TPayload, TLoomMeta, TTurnMeta> {
  ...
  const mint = (
    ...
    atMs = now(),
    author: LoreAuthor = options.author,
    marked?: string,
  ): LoreEventBody => ({
    v: 1,
    id: createId(),
  ...
  return {
    async create(meta) {
      ...
      if (result.status !== "added" && result.status !== "duplicate") {
        throw new Error(`Unable to create lore loom: ${result.status}`);
      }
    ...
    async open(loomId) {
      ...
      return new LoreLoom<TPayload, TLoomMeta, TTurnMeta>(loomId, root, options.store, mint);
    },
    ...
    async import(snapshot) {
      ...
      const newLoomId = `${LORE_PREFIX}${loomEvent.id}`;
...
export function createBrowserLoreLooms<
  ...
  TLoomMeta = unknown,
  TTurnMeta = unknown,
>(idbOptions: IndexedDbEventStoreOptions & Omit<LoreLoomsOptions, "store">): Looms<TPayload, TLoomMeta, TTurnMeta> {
  const { author, now, createId, ...storeOptions } = idbOptions;
  return createLoreLooms({ author, now, createId, store: createIndexedDbEventStore(storeOptions) });
}

class LoreLoom<TPayload, TLoomMeta, TTurnMeta>
...
function foldLoom<TPayload, TLoomMeta, TTurnMeta>(
  ...
  const root = events.find((event) => event.body.kind === "lync/loom" && `${LORE_PREFIX}${event.body.id}` === loomId);
  if (!root) throw unknownLoom(loomId);
...
function eventToLoomInfo<TMeta = unknown>(event: LoreEventBody): LoomInfo<TMeta> {
  return omitUndefined({
    id: `${LORE_PREFIX}${event.id}`,
...
function rootId(id: LoomId): string | null {
  return id.startsWith(LORE_PREFIX) ? id.slice(LORE_PREFIX.length) : null;
}

function validateAuthor(author: LoreAuthor): void {
  if (!author || typeof author.actor !== "string" || author.actor.length === 0) {
    throw new Error("Lore author.actor is required");
  }
}

function compactAuthor(author: LoreAuthor): LoreEventBody["author"] {
  return omitUndefined({
    actor: author.actor,
