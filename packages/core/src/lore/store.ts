...
export abstract class BaseEventStore implements EventStore {
  ...
  async append(ev: LoreEventBody): Promise<AppendResult> {
    return this.ingest(serializeLoreEvent(ev), false);
  }
...
export function serializeLoreEvent(ev: LoreEventBody): string {
...
function parseStoredLine(line: string):
  ...
  const parsed = parseLoreFiles([{ file: "<store>", bytes: `${normalized}\n` }]);
  const diagnostic = parsed.lines[0];
  if (!diagnostic?.event || (diagnostic.class !== "accepted" && diagnostic.class !== "nonconforming")) {
    return { ok: false, reason: diagnostic?.reason ?? "unparseable lore line" };
  }
  return {
    ok: true,
...
function validateKnownLyncEvent(body: LoreEventBody): string | null {
