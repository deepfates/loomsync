...
Draft conformance vectors for `LORE-V0.md`, Part I. These are wrangling fixtures, not a
ratified format for vector metadata.

Each fixture directory contains one or more `.lore` inputs and one `expected.json`.
...
```sh
python3 lore-vectors-draft/generate.py
```
